        ...
        this.client.on('message', async (topic, payload) => {
          try {
            ...
            if (messageType === 'count') {
              ...
              let bus;
              try {
                bus = await this.busModel.findByBusId(busIdStr); 
              } catch (findError) {
                console.error(`Error finding bus by busId: ${findError}`);
                return; // Handle database error appropriately
              }
              ...
              if (!bus) {
                ...
                try {
                  const newBusId = await this.busModel.create({
                    busId: busIdStr,
                    licensePlate: `LP-${busIdStr}`, // Placeholder para licencia
                    capacity: 50, // Valor por defecto
                    status: 'active'
                  });
                  bus = { id: newBusId };
                  console.log(`Nuevo bus creado con ID: ${newBusId}`);
                } catch (createError) {
                  console.error(`Error creating new bus: ${createError.stack || createError}`);
                  return; // Handle database error appropriately
                }
              }
              ...
              if (message.count !== undefined) {
                ...
                try {
                  ...
                  console.error(`Error updating passenger count or creating log: ${updateError}`);
                  return; // Handle database error appropriately
                }
              }
            }
