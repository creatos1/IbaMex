...
class BusModel {
  ...
  async create(busData) {
    try {
      ...
      const result = await request
        ...
        .input('licensePlate', sql.VarChar, busData.licensePlate || `Default-${busData.busId}`)
      ...
      console.error('Error creating bus:', err.stack || err);
      throw err;
    }
  }
  ...
  async findById(id) {
    try {
      ...
      console.error('Error finding bus by id:', err.stack || err);
      throw err;
    }
  }
  ...
  async updatePassengerCount(id, count) {
    try {
      ...
      console.error('Error updating passenger count:', err.stack || err);
      throw err;
    }
  }
  ...
  async logOccupancy(busId, count, location = null) {
    try {
      ...
      console.error('Error logging occupancy:', err.stack || err);
      throw err;
    }
  }
}
